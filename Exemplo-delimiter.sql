...
select p.nome as projeto, p.descricao as descricao from projetos p
...
-- 2 Stored Procedure
delimiter //
create procedure inserir_funcionario(x_nome varchar(255), x_email varchar(255),x_senha varchar(45),x_id_departamento int,x_valor_hora decimal(15,2))
begin 
...
call inserir_funcionario('Rafael','rafa@gmail.com','1021',1,50.00);
...
delimiter //
create function calcular_horas_projetos(id int)
returns int
begin
declare total int;
select sum(fp.horas_trabalhadas) into total from funcionarios_projetos fp where fp.id_projeto = id;
return total;
end // 

delimiter ;

select calcular_horas_projetos(3) as total_horas;

-- Segundo ponto


delimiter //
create function calcular_custo_projeto(id int)
returns int
begin
declare custo_total int;
select sum(fp.horas_trabalhadas * f.valor_hora) into custo_total from funcionarios_projetos fp 
inner join funcionarios f on f.id_funcionario = fp.id_funcionario 
where fp.id_projeto = id;
return custo_total;
end // 

delimiter ;

select calcular_custo_projeto(1) as custo_total;